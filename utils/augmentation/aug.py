...
'''
    ...
    for this image
'''
def transform_img (img, label, flip_left_right=True, flip_up_down=True, 
                   crop=0.75, rot90=True, brightness=0.05, blur=True,
                   contrast=(0.7,0.9), hue=0.06, gamma=0.8,
                   saturation=(0.6,0.9), noise=(0.0,0.05), size=(256,256,3)):
    ...
    if (crop is not None):
        crop_prob = tf.random_uniform([], 0.0, 1.0)
        img, label = tf.cond(tf.less(crop_prob, 0.5),
                                    lambda: (tf.image.central_crop(img, crop), tf.image.central_crop(label, crop)),
                                    lambda: (img, label))
    ...
    if (noise is not None):
        noise_prob = tf.random_uniform([], 0.0, 1.0)
        img = tf.cond(tf.less(noise_prob, 0.5),
                                    lambda: (tf.add(img, tf.random_normal(shape=tf.shape(img), mean=noise[0], stddev=noise[1], dtype=tf.float32))),
                                    lambda: (img))
        
    if (size is not None):
        img = tf.image.resize_images(img, size[0:2])
        label = tf.image.resize_images(label, size[0:2])
