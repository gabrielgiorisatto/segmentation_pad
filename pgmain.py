...
def evaluate_test(model, test_ds, num_test_examples, cspace, epochs, save_model_path=None, type_train='',write_images=True, it=0):
    ...
    np.savetxt('pos_results/' + type_train + cspace + '/' + str(epochs) + '/' + str(it) + '/crf_specificity', crf_specificity)
...
if __name__ == '__main__':
    epochs = 80
    cspacev = ["RGB", "HSV", "LAB", "RGB-HSV", "RGB-HSV-L"]
    img_shapev = [3, 3, 3, 6, 7]
    # cspacev = ["HSV", "LAB"]
    # img_shapev = [3, 3]
    # cspacev = ["RGB"]
    # img_shapev = [3]
    ...
    for cspace, img_dim in zip(cspacev, img_shapev):
        if(cspace == 'RGB-SV-LAB' or cspace == "RGB-HSV-LAB"):
            inicio = 0
            fim = 1
        else:
            inicio = 0
